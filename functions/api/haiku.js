...
function buildSeasonRules(mode) {
  if (mode === 'free') {
    return [
      ...
      '補助キーワードに季語候補がある場合も、写真の視覚情報と合うものだけを使ってください。',
      '便利な雰囲気語に逃げず、光・湿度・生活感・影・少しの違和感に合う言葉を選んでください。',
    ].join(' ');
  }
  ...
  return [
    ...
    '補助キーワードに季語候補がある場合は、写真の光・湿度・生活感・影・少しの違和感と照らし合わせて、一番効く季語を一つだけ選ぶか近い季語に言い換えてください。',
    '一句に季語は一つまでにしてください。',
    '季語だけが浮かないよう、写真の具体物や位置関係と自然につないでください。',
    '露骨に季節を言い当てず、気配として滲ませてください。',
    '珍しい季語を見せびらかさず、しかし「田打ち」「衣がえ」「行水」「夜長」「冬灯」のような生活や身体に近い季語は積極的に使ってよいです。',
    '「花曇」「花曇り」は便利に見えるため、写真に本当に合う場合以外は避けてください。',
...
function buildInstructions(mode) {
  ...
  return [
    ...
    '極端に難解な季語は避けてください。ただし、写真の気配に合う生活寄りの季語や少し古い季語は使ってよいです。',
    'ことばは素直に、静かにしてください。',
...
function buildUserPrompt(note, mode) {
  ...
  if (mode === 'haiku') {
    ...
    parts.push('季節名を直接書かず、写真の気配に合う季語で季節をほのめかしてください。');
    parts.push('難しい単語は使わず、素直な日本語で書いてください。');
    parts.push('写真の説明ではなく、その場の気配が少し残る一句にしてください。');
    parts.push('季語候補がある場合は、候補一覧をそのままなぞらず、写真に合う一語だけを静かに効かせてください。');
  } else {
